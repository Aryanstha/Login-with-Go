...
func login(w http.ResponseWriter, r *http.Request) {
	var tmplt = template.Must(template.ParseFiles("templates/login.tmpl"))
	tmplt.Execute(w, nil)
}
func sign_up(w http.ResponseWriter, r *http.Request) {
	var tmplt = template.Must(template.ParseFiles("templates/sign_up.tmpl"))
	tmplt.Execute(w, nil)
}
func signup_user(w http.ResponseWriter, r *http.Request) {
	...
	if add_user(Username[0], Password[0]) {
		var tmplt = template.Must(template.ParseFiles("templates/layout.tmpl"))
		user := http.Cookie{
		...
		var tmplt = template.Must(template.ParseFiles("templates/error.tmpl"))
		tmplt.Execute(w, nil)
	}
